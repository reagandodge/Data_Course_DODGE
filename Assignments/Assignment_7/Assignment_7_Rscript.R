...
utah = read.csv("../reagandodge/Desktop/Data_Course/Data/Utah_Religions_by_County.csv")
...
buddhist=arrange(buddhist, desc(Pop_2010))
...
# tidy this
a<- utah %>% group_by(Pop.Group)%>% summarise(Religious= mean(Religious))
...
b<-utah %>% group_by(Pop.Group) %>% summarise(Pop_2010 = mean(Pop_2010))
...
religiosity= full_join(a,b)

religiosity # take quick look at resulting table

# order by decreasing population
religiosity = religiosity[order(religiosity$Pop_2010, decreasing = TRUE),]

arrange(religiosity, desc(Pop_2010))
